...
constexpr void iota_delta(InputIterator first, InputIterator last, ValueType x0, const ValueType dx) {
  while (first != last) {
    *first++ = x0;
    x0 += dx;
  }
...
template <class InputIterator, class ValueType>
constexpr void iota_delta(InputIterator first, InputIterator last, ValueType x0, const ValueType dx, std::function<ValueType(ValueType)> f) {
  while (first != last) {
    *first++ = f(x0);
    x0 += dx;
  }
