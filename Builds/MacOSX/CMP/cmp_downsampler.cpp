...
namespace cmp {
static auto computeXStartIdx(const float x_min_lim, const std::vector<float>& x_data) {
  float start_x_index{0};
...
static auto computeXEndIdx(const float x_max_lim, const std::vector<float>& x_data) {
...
static auto calculateXIdxsBetweenStartEnd(const size_t start_x_idx,
                                          const size_t end_x_idx,
